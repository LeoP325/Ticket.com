...
async function removeLegacySeatIndexes() {
  try {
    ...
    if (!(error instanceof Error) || !error.message.includes('ns does not exist')) throw error
  }
}
...
export async function ensureCatalog() {
  ...
  for (const [catalogIndex, item] of catalog.entries()) {
    ...
    const seatingMap = await SeatingMap.findOneAndUpdate(
      { venue: venue._id, name: `標準 ${item.capacity} 席` },
      { $setOnInsert: { description: 'MVP 五排、每排十席配置' } },
      { upsert: true, returnDocument: 'after' },
    )
    ...
    if (catalogIndex === 0) {
      ...
      for (let index = 0; index < 50; index++) {
        await Seat.collection.updateOne(
          { event: event._id, number: index + 1, row: { $exists: false } },
          { $set: { row: String.fromCharCode(65 + Math.floor(index / 10)) } },
        )
      }
    }

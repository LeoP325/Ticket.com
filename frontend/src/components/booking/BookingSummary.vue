  ...
  <v-card class="summary-card" flat>
    ...
    <v-card-text v-if="booked">
      <v-alert
        ...
        variant="tonal"
      >
        您的座位是 {{ seatNumbers.join("、") }} 號。
      </v-alert>
    </v-card-text>

    <v-card-text v-else-if="seatNumbers.length > 0">
      <p class="text-body-2 text-medium-emphasis">城市星光音樂祭 2026</p>
      <p class="seat-number">{{ seatNumbers.join("、") }} 號</p>
      ...
      <p class="text-caption mt-3">
        已選 {{ seatNumbers.length }} 張，最多 2 張。請在倒數結束前確認。
      </p>
    ...
    <v-card-text v-else>
      <p class="text-medium-emphasis">請先從座位圖選擇 1 至 2 個座位。</p>
    </v-card-text>
  </v-card>
</template>
